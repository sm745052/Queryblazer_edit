...
class QueryBlazer {
  ...
  public:
    ...
    std::pair<std::vector<std::pair<std::pair<std::string, float>, int>>, size_t>
    Complete(const std::string &query) {
        std::vector<std::pair<std::pair<std::string, float>, int>> suggestions;
        ...
        auto stable_output_seq =
            Encode(*encoder, encoderMatcher, encoder_begin_state, ilabels,
                   false, &encoder_state);
        // EDIT
        int stable_output_seq_len = stable_output_seq.size();
        ...
        for (const auto &candidate : autocomplete.first) {
            ...
            // EDIT
            int candidate_subwords_len = stable_output_seq_len + candidate.first.size();
            ...
            suggestions.push_back({{output, init_cost + candidate.second}, candidate_subwords_len});
        }
